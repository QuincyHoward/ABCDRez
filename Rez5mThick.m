...
function [www,wthetaL0,FlagRez]=Rez5mThick(lambda,RezPara,str)
if nargin==2
    str='';
end
...
nF3=     RezPara(13);d2=    RezPara(14);rho2=   RezPara(15);
%
syms z
%总腔长
LL=d1+lenF1+d3+lenF2+d4+lenF3+d2;
...
%
MM=[1,d2;0,1]*MF3*[1,d4;0,1]*MF2*[1,d3;0,1]*MF1*[1,d1;0,1];
...
if ( FlagRez(1)<0  ||  FlagRez(1)>1 )
    ...
    [w002,theta002,L002]=fLRMm(wf003,thetaf003,Lf003,lambda,MBack3,0,d1+lenF1+d3+lenF2+d4+lenF3);
    %
    ...
    www=1/6*double(subs(w001_z,z,d1)+subs(w003_z,z,d1+lenF1)+...
        subs(w003_z,z,d1+lenF1+d3)+subs(w004_z,z,d1+lenF1+d3+lenF2)+...
        subs(w004_z,z,d1+lenF1+d3+lenF2+d4)+subs(w002_z,z,d1+lenF1+d3+lenF2+d4+lenF3));
    ...
    if strcmp(str,'plot')
        ...
        fplot(w002_z,'m',[d1+lenF1+d3+lenF2+d4+lenF3,d1+lenF1+d3+lenF2+d4+lenF3+d2]);
        % fplot(w002s_z,'k*',[d1+lenF1+d3+lenF2+d4+lenF3,d1+lenF1+d3+lenF2+d4+lenF3+d2]);
