...
wwwFF=zeros(length(d4x),length(ffx),length(rho1x));
wwwF3=zeros(length(d4x),length(ffx),length(rho1x));
wwwFFmax=zeros(length(ffx),length(rho1x));
wwwF3max=zeros(length(ffx),length(rho1x));
ParaRezs=[];
%% 暴力求解，耗时长
tic
for ii=1:length(rho1x)
    ...
    for jj=1:length(ffx)
        ...
        for ll=1:length(d4x)
            ...
            wwwFF(ll,jj,ii)=www(1);
            wwwF3(ll,jj,ii)=www(2);

            Flagwww=FunFlagwww(www,limR,limRF3);
            if Flagwww
                ParaRezs=[ParaRezs;rho1,ff1,d4,www(1),www(2)];
                disp(string(sprintf('[rho1,ff1,d4,www]=[%.3f,%.3f,%.3f,{%.3f,%.3f}]\n',...
                    rho1,ff1,d4,www(1)*10^6,www(2)*10^6)));
                figure;
                [~,wthetaL0,FlagRez]=Rez7mThick(lambda,RezPara,'plot');

            end


        end
        [wwwFFmax(jj,ii),numll]=(max(wwwFF(:,jj,ii)));
        wwwF3max(jj,ii)=(wwwF3(numll,jj,ii));

        disp(string(sprintf('\t\t\t[rho1,ff1,wwwmax]=[%.3f,%.3f,\t{%.3f,%.3f}]\n',...
            rho1,ff1,10^6*wwwFFmax(jj,ii),10^6*wwwF3max(jj,ii))));
